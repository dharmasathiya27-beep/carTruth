      ...
      <div className="space-y-4">
        {history.map((record, index) => {
          ...
          return (
            <div
              key={index}
              className="flex gap-6 pb-6 border-b border-white/10 last:border-b-0 last:pb-0"
            >
              ...
              <div className="flex-1 pt-1">
                  ...
                  <div className="mb-3 rounded-lg border border-red-500/25 bg-red-500/10 p-3">
                    ...
                    <div className="space-y-1">
                      {failures.map((failure) => (
                        <p key={failure} className="text-xs text-red-100">
                          {failure}
                        </p>
                      ))}
                    </div>
                ...
                {classified.length > 0 && (
                  <div className="mt-3 space-y-2">
                    {classified.map((defect, idx) => (
                      <div
                        key={idx}
                        className="bg-slate-900/50 border border-white/10 rounded-lg p-3"
                      >
                        <div className="flex flex-wrap items-center gap-2 mb-2">
                          ...
                          <span
                            className={`px-2 py-1 rounded border text-xs font-semibold ${severityClass(defect.severity)}`}
                          >
                            {defect.severity}
                          </span>
                          {defect.is_repeated && (
                ...
                {classified.length === 0 && (
                  <p className="mt-3 text-sm text-emerald-300">
                    No advisories or failure reasons recorded.
                  </p>
                )}
              </div>
            </div>
